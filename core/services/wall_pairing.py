...
from core.services.types import Face3D, Vec3, dot, normalize, vlength
...
class TwinCandidate:
    ...
    centroid: Vec3
    extent: (
        float  # dimensión máxima del bounding box (presupuesto de solapamiento lateral)
    )


def are_thin_twins(
    a: TwinCandidate, b: TwinCandidate, thickness_threshold: float
) -> Optional[float]:
    """
...
def cluster_verticals(
    faces: List[Face3D], vertical_indices: List[int]
    ...
    for i in vertical_indices:
        ...
        for cl in clusters:
            if (
                ...
                and abs(d - cl.d) < COPLANAR_D_TOLERANCE
            ):
        ...
        if not placed:
            clusters.append(VerticalCluster(normal=n, d=d, face_indices=[i]))
...
def find_twin_thickness(
    cluster: VerticalCluster, all_clusters: List[VerticalCluster]
) -> Optional[float]:
...
def find_thin_wall_faces(
    faces: List[Face3D], vertical_indices: List[int], thickness_threshold: float
) -> Set[int]:
    """
